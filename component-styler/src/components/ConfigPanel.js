      ...
      {isOpen && (
        <div className="config-drawer">
          ...
          <div className="config-save-row">
            ...
            />
            <button onClick={handleSave} className="save-btn" disabled={!saveName.trim()}>
              Save
            </button>
          </div>
          ...
          <div className="config-list">
            {filtered.length === 0 && (
              <div className="config-empty">
                {configs.length === 0 ? 'No saved configurations yet.' : 'No results found.'}
              </div>
            )}
            {filtered.map(config => (
              <div key={config.id} className="config-item">
                ...
                <div className="config-item-actions">
                  <button onClick={() => onLoad(config)} className="load-btn" title="Load">
                    📂
                  </button>
                  <button onClick={() => onDelete(config.id)} className="del-btn" title="Delete">
                    🗑️
                  </button>
                </div>
              </div>
